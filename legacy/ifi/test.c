...
#define FNTNAME1 "\\PSFONTS\\ARIAL.TTF"
#ifdef USE_ATM
...
FILE *f;
   ...
   printf("IFIMETRICS structure is %d bytes\n\n", sizeof(IFIMETRICS) );
...
//   rc = fdhdr.pfddisp->FdUnloadFontFile(hff);
   ...
   printf("IFIMETRICS.fsType         = 0x%X\n", ifimet[0].fsType );
   printf("IFIMETRICS.fsDefn         = 0x%X\n", ifimet[0].fsDefn );
   printf("IFIMETRICS.fsSelection    = 0x%X\n", ifimet[0].fsSelection );
   printf("IFIMETRICS.fsCapabilities = 0x%X\n", ifimet[0].fsCapabilities );
   printf("IFIMETRICS.ulReserved     = 0x%X\n", ifimet[0].ulReserved );
   printf("IFIMETRICS.panose         = {%u,%u,%u,%u,%u,%u,%u,%u,%u,%u}\n",
          ifimet[0].panose[0], ifimet[0].panose[1], ifimet[0].panose[2], ifimet[0].panose[3],
          ifimet[0].panose[4], ifimet[0].panose[5], ifimet[0].panose[6], ifimet[0].panose[7],
          ifimet[0].panose[8], ifimet[0].panose[9], ifimet[0].panose[10]
         );

#if 0
f = fopen("test.log", "wb");
fwrite( ifimet, sizeof(IFIMETRICS), 1, f );
fclose( f );
#endif

   printf("Querying faces... RC = %X\n", rc);
   printf("Glyphlist is %s\n", ifimet[0].szGlyphlistName);
   printf("Underscore size is %d\n", ifimet[0].lUnderscoreSize);
