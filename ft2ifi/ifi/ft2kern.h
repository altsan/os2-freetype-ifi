...
typedef struct  TT_Kern_2_Class_
{
    ...
    FT_UShort   classes[1];     /* a table giving for each ranged glyph    */
...
typedef struct TT_Kern_2_
{
    ...
    FT_FWord         array[1];       /* 2-dimensional kerning values array */
} FX_Kern_2;
...
typedef struct _Kerning_Table {
    ...
    FT_Byte             tables[1];   /* start of subtable data */
} FX_Kerning;
...
FT_Error FX_Get_Kerning_Pairs( FT_Face face, FX_Kern_0 **kern0 );
#if 0
